...
async function capture(config: BrowserLlmConfig, kind: "analyze" | "review" | "diagnose" = "analyze") {
  const fetchMock = vi.fn().mockResolvedValue(completion());
  vi.stubGlobal("fetch", fetchMock);
  await requestValidatedJson(kind, { system: "Return JSON", user: "Input" }, outputSchema, 123, config);
...
describe("provider request compatibility", () => {
  it("uses strict schema and max_completion_tokens for OpenAI", async () => {
    const request = await capture({ apiKey: "test", provider: "openai", baseURL: "https://api.openai.com/v1", analyzeModel: "gpt-5.6-sol", reviewModel: "gpt-5.6-sol", diagnoseModel: "gpt-5.6-sol" });
  ...
  it("uses JSON Object mode and disables thinking for DeepSeek", async () => {
    const request = await capture({ apiKey: "test", provider: "deepseek", baseURL: "https://api.deepseek.com", analyzeModel: "deepseek-v4-flash", reviewModel: "deepseek-v4-flash", diagnoseModel: "deepseek-v4-flash" });
  ...
  it("uses the conservative OpenAI-compatible relay request", async () => {
    const request = await capture({ apiKey: "test", provider: "custom", baseURL: "https://relay.example", analyzeModel: "relay-model", reviewModel: "relay-model", diagnoseModel: "relay-model" });
    expect(request.url).toBe("https://relay.example/v1/chat/completions");
    expect(request.body.max_tokens).toBe(123);
    expect(request.body.response_format).toEqual({ type: "json_object" });
  });

  it("uses the independently configurable review model", async () => {
    const request = await capture({ apiKey: "test", provider: "custom", baseURL: "https://relay.example", analyzeModel: "draft-model", reviewModel: "review-model", diagnoseModel: "diagnose-model" }, "review");
    expect(request.body.model).toBe("review-model");
  });
  ...
  it("serializes and validates the refined full diagnosis schema", async () => {
    ...
    const result = await requestValidatedJson("diagnose", { system: "Return JSON", user: "Input" }, createDiagnosisSchema(questions, answers), 6000, { apiKey: "test", provider: "openai", baseURL: "https://api.openai.com/v1", analyzeModel: "gpt-4.1-mini", reviewModel: "gpt-4.1-mini", diagnoseModel: "gpt-4.1-mini" });
